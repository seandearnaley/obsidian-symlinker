...
function getObsidianConfigPath() {
	...
	switch (process.platform) {
		case "win32": {
			...
			possiblePaths.push(path.join(app.getAppPath(), "..", "Data", "obsidian", "obsidian.json"));
...
function searchForVaultsByDirectory() {
	...
	console.log(`Found ${potentialVaults.length} potential Obsidian vaults by directory scanning`);
...
ipcMain.handle("create-symlink", async (event, { targetFiles, vaultPath }) => {
	...
	console.log("Raw data received:", JSON.stringify({ targetFiles, vaultPath }, null, 2));

	const results = [];

	for (const fileObj of targetFiles) {
		try {
			console.log("\nProcessing file object:", JSON.stringify(fileObj, null, 2));
			...
			console.log("Successfully created symlink from", filePath, "to", symlinkPath);
			results.push({
				success: true,
				file: targetFileName,
				targetPath: filePath,
