...
function selectVaultByPath(targetPath) {
  ...
  if (!found && targetPath) {
    console.log(
      `Path ${targetPath} not found in selector, treating as custom vault`
    );
...
createSymlinksBtn.addEventListener("click", async () => {
  ...
  const filesToProcess = selectedFiles.map((file) => {
    console.log(
      `Processing file: ${file.originalName}, Custom name: ${
        file.customName || "NONE"
      }`
    );
    return {
